 ...
 *
 * Faehrt die Bahn aus robot_path.txt ab .
 *
 * Aufbau der Datei:
 *   Kopfzeilen  "# SCHLUESSEL=WERT"  (u. a. NUR_LUFTLAUF und die
 *               Geschwindigkeit je Phase, z. B. V_CONTACT=10.0)
 *   Posenzeilen "PHASE;X;Y;Z;A;B;C"  (mm / Grad, Dezimalpunkt)
 *
 * Sicherheit:
 *   - Steht im Kopf NUR_LUFTLAUF=1 (Kalibrierung unzureichend),
 *     erscheint vor dem Start ein deutlicher Warn-Dialog.
...
public class MarkierungExecutor extends RoboticsAPIApplication {
    ...
    public void run() {
        ...
        boolean nurLuftlauf = "1".equals(wert(kopf, "NUR_LUFTLAUF", "1"));
        if (nurLuftlauf) {
            int w = getApplicationUI().displayModalDialog(
                    ApplicationDialogType.WARNING,
                    "NUR_LUFTLAUF=1 — die Kalibrierung ist unzureichend ("
                            + wert(kopf, "KALIBRIERUNG", "unbekannt")
                            + ").\nDie Bahn wurde OHNE Bauteilkontakt "
                            + "geplant und wird nur mit Sicherheitsabstand "
                            + "abgefahren.",
                    "Luftlauf starten", "Abbrechen");
            if (w != 0) {
                getLogger().info("Abbruch durch Bediener (Luftlauf-Dialog).");
                return;
            }
        }
        ...
        getLogger().info("Bahn vollstaendig abgefahren ("
                + (nurLuftlauf ? "Luftlauf" : "Markierung") + ").");
    }
